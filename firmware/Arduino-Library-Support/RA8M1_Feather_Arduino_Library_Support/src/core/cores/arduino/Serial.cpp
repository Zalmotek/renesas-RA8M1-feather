...
/* -------------------------------------------------------------------------- */
  if(init_ok) {
    tx_done = false;
    R_SCI_B_UART_Write(&uart_ctrl, &c, 1);
    while (!tx_done) {}
    return 1;
  }
  else {
...
size_t  UART::write(uint8_t* c, size_t len) {
  if(init_ok) {
    tx_done = false;
    R_SCI_B_UART_Write(&uart_ctrl, c, len);
    while (!tx_done) {}
    return len;
  }
  else {
...
/* -------------------------------------------------------------------------- */
  ...
  if(init_ok) {
    if (channel == 2) {
        uart_ctrl = g_uart_ctrl;
        uart_cfg = g_uart_cfg;
    }
    else if (channel == 0) {
        uart_ctrl = g_uart0_ctrl;
        uart_cfg = g_uart0_cfg;
    }
    else if (channel == 4) {
        uart_ctrl = g_uart4_ctrl;
        uart_cfg = g_uart4_cfg;
    }

    UART::g_uarts[channel]        = this;

    uart_baud.baudrate_bits_b.abcse          = 0;
    uart_baud.baudrate_bits_b.abcs           = 0;
    uart_baud.baudrate_bits_b.bgdm           = 1;
    uart_baud.baudrate_bits_b.cks            = 0;
    uart_baud.baudrate_bits_b.brr            = 25;
    uart_baud.baudrate_bits_b.mddr           = (uint8_t) 256;
    uart_baud.baudrate_bits_b.brme           = false;

    uart_cfg_extend.clock                         = SCI_B_UART_CLOCK_INT;
    uart_cfg_extend.rx_edge_start                 = SCI_B_UART_START_BIT_FALLING_EDGE;
    uart_cfg_extend.noise_cancel                  = SCI_B_UART_NOISE_CANCELLATION_DISABLE;
    uart_cfg_extend.rx_fifo_trigger               = SCI_B_UART_RX_FIFO_TRIGGER_MAX;
    uart_cfg_extend.p_baud_setting                = &uart_baud;
    uart_cfg_extend.flow_control                  = SCI_B_UART_FLOW_CONTROL_RTS;
    uart_cfg_extend.flow_control_pin              = (bsp_io_port_pin_t) UINT16_MAX;
    if (rts_pin != -1 && cts_pin != -1) {
      uart_cfg_extend.flow_control                  = SCI_B_UART_FLOW_CONTROL_HARDWARE_CTSRTS;
    }
    uart_cfg_extend.rs485_setting.enable          = SCI_B_UART_RS485_DISABLE;
    uart_cfg_extend.rs485_setting.polarity        = SCI_B_UART_RS485_DE_POLARITY_HIGH;
  ...
  err = R_SCI_B_UART_BaudCalculate(baudrate, bit_mod, err_rate, &uart_baud);
  if (uart_baud.baudrate_bits_b.mddr == 0) {
    err = R_SCI_B_UART_BaudCalculate(baudrate, false, err_rate, &uart_baud);
  }
  err = R_SCI_B_UART_BaudSet(&uart_ctrl, (void *) &uart_baud);
  if(err != FSP_SUCCESS) while(1);
  err = R_SCI_B_UART_Open (&uart_ctrl, &uart_cfg);
  if(err != FSP_SUCCESS) while(1);
...
/* -------------------------------------------------------------------------- */
  ...
  while (i < len) {
    ...
    while (uart_ctrl.p_reg->CSR_b.TEND == 0) {}
    i++;
  }
  return len;
}
