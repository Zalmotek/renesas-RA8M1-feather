...
class GPTimer {
  public:
    ...
    GPTimer(timer_cfg_t &cfg) {
        ...
        else if (cfg.channel == 5) {
            ctrl = g_timer4_ctrl;
            aux = g_timer4_cfg;
        }

        aux.channel              = cfg.channel;
        aux.p_callback           = cfg.p_callback;
        aux.period_counts        = cfg.period_counts;
        aux.duty_cycle_counts    = cfg.duty_cycle_counts;
        aux.mode                 = cfg.mode;
        aux.source_div           = cfg.source_div;
        cfg = aux;
