...
#include "../../cores/arduino/IRQManager.h"
...
static spi_event_t _spi_cb_event[SPI_MAX_SPI_CHANNELS] = {SPI_EVENT_TRANSFER_ABORTED};
...
void ArduinoSPI::begin()
{
  ...
  {
    _open               = R_SPI_B_Open;
    _close              = R_SPI_B_Close;
    _write_then_read    = R_SPI_B_WriteRead;
  ...
  _spi_ext_cfg.spi_clksyn         = SPI_B_SSL_MODE_CLK_SYN;
  _spi_ext_cfg.spi_comm           = SPI_B_COMMUNICATION_FULL_DUPLEX;
  _spi_ext_cfg.ssl_polarity       = SPI_B_SSLP_LOW;
  _spi_ext_cfg.ssl_select         = SPI_B_SSL_SELECT_SSL0;
  _spi_ext_cfg.mosi_idle          = SPI_B_MOSI_IDLE_VALUE_FIXING_DISABLE;
  _spi_ext_cfg.parity             = SPI_B_PARITY_MODE_DISABLE;
  _spi_ext_cfg.byte_swap          = SPI_B_BYTE_SWAP_DISABLE;
  _spi_ext_cfg.spck_div.spbr      = 1; /* Actual calculated bitrate: 12000000. */
  _spi_ext_cfg.spck_div.brdv      = 0;
  _spi_ext_cfg.spck_delay         = SPI_B_DELAY_COUNT_1;
  _spi_ext_cfg.ssl_negation_delay = SPI_B_DELAY_COUNT_1;
  _spi_ext_cfg.next_access_delay  = SPI_B_DELAY_COUNT_1;
...
uint8_t ArduinoSPI::transfer(uint8_t data)
{
    ...
    else
    {
        ...
        delayMicroseconds(350); // Nicu's addition
    ...
    if (_is_sci) {
        ...
        */
        ...
        if (n32 != 0U) {
            ...
            _spi_ctrl.p_regs->SPCMD0_b.SPB = 0x1F; /* spi bit width = 32 */
            ...
            _spi_ctrl.p_regs->SPCMD0_b.SPB = 0x07; /* spi bit width = 8 */
            _spi_ctrl.p_regs->SPCR_b.SPE = 1; /* enable SPI unit */
        }
        ...
        for (uint8_t index = 0; index < bytes_remaining; index++) {
            _spi_ctrl.p_regs->SPDR = buffer[index];
            while (0U == _spi_ctrl.p_regs->SPSR_b.SPRF) {}
            delayMicroseconds(350); // Nicu's addition
            buffer[index] = _spi_ctrl.p_regs->SPDR;
        }
    }
}
...
#define SPI_REG(channel)  ((R_SPI_B0_Type *) ((uint32_t) R_SPI_B0 + \
                           ((uint32_t) R_SPI_B1 - (uint32_t) R_SPI_B0) * (channel)))
    ...
    R_SPI_B_CalculateBitrate(settings.getClockFreq(), SPI_B_CLOCK_SOURCE_PCLK, &spck_div);
    ...
    _spi_ctrl.p_regs->SPCR3 = (uint8_t) sslp;

    uint32_t sppcr  = 0;
    /* set MOSI idle value to low */
    //sppcr |= R_SPI0_SPPCR_MOIFE_Msk;
    _spi_ctrl.p_regs->SPCR2_b.MOIFE = (uint8_t) 1;

    /* configure bit rate */
    _spi_ctrl.p_regs->SPCR3_b.SPBR = (uint8_t) spck_div.spbr;
    ...
    _spi_ctrl.p_regs->SPDCR_b.BYSW = ~bit_order;

    _spi_ctrl.p_regs->SPDECR_b.SCKDL = 0;

    _spi_ctrl.p_regs->SPDECR_b.SLNDL = 0;

    _spi_ctrl.p_regs->SPDECR_b.SPNDL = 0;

    _spi_ctrl.p_regs->SPCR_b.SCKASE = 1;
    ...
    //_spi_ctrl.p_regs->SPCMD0 = 0;
    _spi_ctrl.p_regs->SPCMD0_b.SSLA = 0;
    _spi_ctrl.p_regs->SPCMD0_b.SPNDEN = 0;
    _spi_ctrl.p_regs->SPCMD0_b.SLNDEN = 0;
    _spi_ctrl.p_regs->SPCMD0_b.SCKDEN = 0;
    _spi_ctrl.p_regs->SPCMD0_b.CPHA = clk_phase;
    _spi_ctrl.p_regs->SPCMD0_b.CPOL = clk_polarity;
    _spi_ctrl.p_regs->SPCMD0_b.BRDV = spck_div.brdv; /* set bit rate division */
    _spi_ctrl.p_regs->SPCMD0_b.SPB = 0x07; /* spi bit width = 8 */
    _spi_ctrl.p_regs->SPCMD0_b.LSBF = bit_order;

    _spi_ctrl.p_regs->SPSR; /* read to clear OVRF */
    //_spi_ctrl.p_regs->SPSR = 0;
