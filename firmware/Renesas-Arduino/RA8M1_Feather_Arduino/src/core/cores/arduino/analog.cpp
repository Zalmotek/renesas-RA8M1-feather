...
ADC_Container::ADC_Container(uint8_t unit, ADCCbk_f cbk, void *ctx /*= nullptr*/) {
  ...
  cfg_extend.trigger              = ADC_START_SOURCE_DISABLED;
  cfg_extend.trigger_group_b      = ADC_START_SOURCE_DISABLED;
  cfg_extend.double_trigger_mode  = ADC_DOUBLE_TRIGGER_DISABLED;
  cfg_extend.adc_vref_control     = ADC_VREF_CONTROL_VREFH;
...
void analogWrite(pin_size_t pinNumber, int value)
{
  ...
  if(ptr != nullptr) {
    bool suc = ptr->pulse_perc((float)value * 100.0 / (1 << _writeResolution));
  }
}
