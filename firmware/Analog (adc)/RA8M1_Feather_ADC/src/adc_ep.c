...
fsp_err_t adc_read_data(void)
{
    ...
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_4, &g_adc_data3);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_7, &g_adc_data4);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_8, &g_adc_data5);
...
//    APP_PRINT("\r\nThe Voltage Reading from ADC: %d\r\n", g_adc_data);
    ...
    g_adc_volt = (float)((g_adc_data * V_REF)/ADC_12_BIT);
    snprintf((char *)g_volt_str,SIZE_64, "%0.2f", g_adc_volt);
    APP_PRINT("\r\nThe ADC input voltage CH0: %s\r\n", g_volt_str);

    g_adc_volt = (float)((g_adc_data1 * V_REF)/ADC_12_BIT);
    snprintf((char *)g_volt_str,SIZE_64, "%0.2f", g_adc_volt);
    APP_PRINT("\r\nThe ADC input voltage CH1: %s\r\n", g_volt_str);

    g_adc_volt = (float)((g_adc_data2 * V_REF)/ADC_12_BIT);
    snprintf((char *)g_volt_str,SIZE_64, "%0.2f", g_adc_volt);
    APP_PRINT("\r\nThe ADC input voltage CH2: %s\r\n", g_volt_str);

    g_adc_volt = (float)((g_adc_data3 * V_REF)/ADC_12_BIT);
    snprintf((char *)g_volt_str,SIZE_64, "%0.2f", g_adc_volt);
    APP_PRINT("\r\nThe ADC input voltage CH4: %s\r\n", g_volt_str);

    g_adc_volt = (float)((g_adc_data4 * V_REF)/ADC_12_BIT);
    snprintf((char *)g_volt_str,SIZE_64, "%0.2f", g_adc_volt);
    APP_PRINT("\r\nThe ADC input voltage CH7: %s\r\n", g_volt_str);

    g_adc_volt = (float)((g_adc_data5* V_REF)/ADC_12_BIT);
    snprintf((char *)g_volt_str,SIZE_64, "%0.2f", g_adc_volt);
    APP_PRINT("\r\nThe ADC input voltage CH8: %s\r\n", g_volt_str);
